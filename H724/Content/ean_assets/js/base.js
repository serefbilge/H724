    ...
    _renderMenu: function (e, d) {
        ...
        $.each(d, function(d, b) {
            if (b.category != f) {
                ...
                var a = i.template,
                    a = a.replace(/\{categoryClass\}/g, g),
                    a = a.replace(/\{categoryIcon\}/g, c),
                    a = a.replace(/\{categoryName\}/g, b.category);
...
Function.prototype.method = function(a, b) {
    if (this.prototype[a])
        throw { name: "Method Overwrite Error", message: "There is already a property named '" + a + "' on " + this };
...
var datepickerUtils = {
    ...
    updateDatesHandler: function (a, b, c) {
        return function(d) {
            ...
            try {
                e = $.datepicker.parseDate(a.datepicker("option", "dateFormat"), d);
            } catch(g) {
            }
            ...
            if (e) {
                ...
                if (!datepickerUtils.checkoutChanged || f <= e)
                    d = new Date(e), d.setDate(d.getDate() + c.initialNights);
...
$.widget("custom.synchronizedDatepicker", {
    _init: function () {
        ...
        b.datepicker("option", {
            minDate: a.minDate,
            maxDate: a.maxDate,
            onSelect: datepickerUtils.updateDatesHandler(b, c, a)
        }).change(datepickerUtils.updateDatesHandler(b, c, a));

        var d = datepickerUtils.getCheckoutLimits(b.datepicker("getDate"), a);

        c.datepicker("option", {
            minDate: d.min, maxDate: d.max, onSelect: function () {
...
var urlParameters = {
    objectFromQueryString: function(b) {
    ...
    queryStringFromObject: function(b) {
        ...
        if ($.isArray(b[c]))
            for (var d = 0; d < b[c].length; d++)
                a.push(c + "=" + encodeURIComponent(b[c][d]));
        else b[c] && a.push(c + "=" + encodeURIComponent(b[c]));
...
$(document).ready(function () {
    ...
    datepickerUtils.datepickerOptions = {
        other: g,
        minNights: 1,
        maxNights: 28,
        initialNights: 2,
    ...
    f.change(function () {
        datepickerUtils.synchronizeDates(f, u);
    });
    ...
    $("#lang, #currency").change(function (a) {
        ...
        -1 < window.location.pathname.indexOf("/templates/cindex/") ? window.location.pathname = appConfig.landing.baseCachedUrl + $("#currency").val() + "/" + $("#lang").val() : window.location.search = urlParameters.updateQueryString(window.location.search, b);
    });

    var e = $("#rooms-and-guests").contents().filter(function() {
        return 8 === this.nodeType;
    }),
        w = 0 < e.length ? e[0].nodeValue : null,
        s = function(a) {
                var b = w,
                    b = b.replace(/\{roomNumber\}/g, a + 1);
        ...
        o = function (a) {
                ...
                a && (
                    ...
                    8 <= b.length ? (a.attr("disabled", !0),
                    a.addClass("disabled")) : 1 >= b.length && b.find(".remove-row").remove();

                    1 < b.length && (
                        a = $('<button class="remove-row">X</button>').click(function (a) {
                                                                                a.preventDefault();
                                                                                a = $(this).closest("li");
    ...
    c.bind("keypress", function () { q = !1; });
    c.bind("categoryautocompleteselect", function () { q = !0; });
    
    k = {
        wasAutocompleted: function () { return q; },
        hasValue: function () { return 0 < c.not(":hidden").length && c.val(); },
        value: function () { return c.val(); }
    };

    c.change(function () { h.val(""); });

    var t = function () { c.keyup(function(a) { 13 === a.which && $("#SearchBox_Submit").click(); }); };

    t();

    var x = appConfig.landing.baseUrl + "destination",
        e = $("#SearchBox_DestinationAutoSuggest").contents().filter(function () { return 8 === this.nodeType; });
    
    c.categoryAutocomplete({
        delay: 0,
        appendTo: "#SearchBox_DestinationAutoSuggest",
        source: function(a, b) {
            $.ajax({
                url: x,
                data: { propertyName: a.term },
                dataType: "json",
    ...
    $("#SearchBox_Submit").click(function (a) {
        ...
        if (y()) {
            if (k.hasValue() && !k.wasAutocompleted() && k.value() !== $("#search-box-initial-destination").val()) {
                var a = appConfig.landing.baseUrl + "destination/locByDest?propertyName=" + encodeURIComponent(c.val()),
                    b = function (a) {
                        a = a.split("|");
                ...
                d.bind("dialogclose", function () { d.remove("ul"); });
                ...
                $.ajax({ url: a, dataType: "json" }).done(function(a) {
                    if (1 < a.items.length) {
                        ...
                        for (var f = 0; f < a.items.length; f++) {
                            ...
                            c.append($("<li/>")
                                .append($("<label/>", { "for": h })
                                    .append($("<input/>", { type: "radio", name: "destination", "class": "radio", id: h, value: g.name }))
                                    .append(g.name)));
                        }
    ...
    var y = function () {
        ...
        0 < c.length && 0 >= c.val().length && (a = !1, l.addClass("errorLabel"), i.addClass("errorField"), i.focus());
        ...
        $.each([f, g], function() {
            ...
            !$("#dateless").is(":checked") &&
            !datepickerUtils.isInRange(this, datepickerUtils.datepickerOptions) &&
            (a || $(this).focus(), a = !1, $(this).addClass("errorField"), $(this).closest("label").addClass("errorLabel"));
        });

        var b = !1;

        $("#rooms-and-guests > li").filter(function () { return !$(this).hasClass("add-row"); })
                                   .each(function () {
                                       $(this).find(".children-ages select:enabled")
                                              .each(function () {
                                                        var c = $(this);
